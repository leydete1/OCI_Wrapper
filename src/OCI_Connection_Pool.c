                     ...
                     logger_t          *logger)
{
    ...
    /* ---- Per-slot server handle + attach (2026-08-06) ----
     * Each slot gets its own genuinely independent physical connection
     * now, rather than attaching to one connection shared by every
     * slot - see pool_slot_t's own doc comment in OCI_Connection_Pool.h
     * for the full story (a real ORA-03114 cascade under concurrent
     * worker threads, traced to the shared-connection design). This is
     * an extra OCIServerAttach() (and its own TCP handshake) per slot
     * rather than once for the whole pool - the necessary cost of
     * genuine per-slot isolation.                                      */
    CHECK_OCI_POOL(slot->errhp,
        OCIHandleAlloc(pool->envhp, (void **)&slot->srvhp,
                       OCI_HTYPE_SERVER, 0, NULL),
        logger, Cleanup);

    logger_write(logger, LOG_INFO, __func__, 0,
                 "Calling OCIServerAttach for this slot, dbname='%s'",
                 pool->dbname);
    CHECK_OCI_POOL(slot->errhp,
        OCIServerAttach(slot->srvhp, slot->errhp,
                        (text *)pool->dbname,
                        (sb4)strlen(pool->dbname),
                        OCI_DEFAULT),
        logger, Cleanup);
    logger_write(logger, LOG_INFO, __func__, 0,
                 "OCIServerAttach OK for this slot");

    /* ---- Bound how long any call on THIS slot's own connection will
     * wait for a response (including a call blocked on another
     * session's row lock). Safe to set per-slot now that each slot has
     * its own independent physical connection - see the header comment
     * above for why this previously caused problems when applied to a
     ...
     * (older Oracle Client) - logged as WARN, slot still opens.        */
    {
        ...
        sword recv_rc = OCIAttrSet(slot->srvhp, OCI_HTYPE_SERVER,
                                    &recv_timeout, 0,
                                    OCI_ATTR_RECEIVE_TIMEOUT, slot->errhp);
        if (recv_rc != OCI_SUCCESS && recv_rc != OCI_SUCCESS_WITH_INFO)
            logger_write(logger, LOG_WARN, __func__, 0,
                         "OCIAttrSet OCI_ATTR_RECEIVE_TIMEOUT failed (rc=%d) "
                         "for this slot - blocked calls on it will NOT time "
                         "out client-side (requires Oracle Client 18c+)",
                         (int)recv_rc);
        else
            logger_write(logger, LOG_INFO, __func__, 0,
        ...
        sword send_rc = OCIAttrSet(slot->srvhp, OCI_HTYPE_SERVER,
                                    &send_timeout, 0,
                                    OCI_ATTR_SEND_TIMEOUT, slot->errhp);
    ...
    /* ---- Attach THIS slot's own server handle to its service context
     * (was pool->srvhp - the one shared connection - before 2026-08-06) */
    CHECK_OCI_POOL(slot->errhp,
...
Cleanup:
    ...
    if (slot->srvhp)
    {
        OCIServerDetach(slot->srvhp, slot->errhp, OCI_DEFAULT);
        OCIHandleFree(slot->srvhp, OCI_HTYPE_SERVER);
        slot->srvhp = NULL;
    }
...
static void close_slot(pool_slot_t *slot, logger_t *logger)
{
    ...
    /* Detach/free this slot's own server connection (2026-08-06 - was
     * pool-level and only detached once for the whole pool before).
     * Must happen before errhp is freed below - OCIServerDetach needs
     * a valid error handle to report any error from the detach itself. */
    if (slot->srvhp)
    {
        OCIServerDetach(slot->srvhp, slot->errhp, OCI_DEFAULT);
        OCIHandleFree(slot->srvhp, OCI_HTYPE_SERVER);
...
#undef COPY_NLS
    ...
    /* ---- Pool-level error handle (init-time / non-slot-specific
     * operations only - see OCI_Connection_Pool.h) ---- */
    CHECK_OCI_POOL(pool->envhp,
        OCIHandleAlloc(pool->envhp, (void **)&pool->errhp,
                       OCI_HTYPE_ERROR, 0, NULL),
        ctx->connectionpool_logger, Cleanup);

    /* No pool-level server handle/attach any more (2026-08-06) - each
     * slot now does its own OCIServerAttach() inside open_slot(), a
     * genuinely independent physical connection per slot rather than
     * one shared by all of them. See OCI_Connection_Pool.h's own
     * comment on pool_slot_t for the full story (ORA-03114 cascade
     * under real concurrent worker threads, traced to the shared
     * connection). */
...
Cleanup:
    ...
    /* No pool-level srvhp to detach/free any more - close_slot() above
     * already detached each open slot's own server connection. */
...
void OCI_Disconnect_pool(oci_context_t *ctx)
{
    ...
    /* No pool-level srvhp to detach/free any more (2026-08-06) -
     * close_slot() above already detached each open slot's own server
