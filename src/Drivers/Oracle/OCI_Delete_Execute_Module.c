                          ...
                          execute_config_t  *cfg)
{
    ...
    /* v2 driver integration (2026-09-16). The old, separate
     * OCIStmtPrepare2 call that used to sit here is gone - dml_execute()
     * does prepare/bind/execute/rowcount/release as one atomic call
     * (see db_driver.h's own doc comment for why: no cursor to keep
     * alive across calls the way SELECT genuinely needs, so there is
     * nothing to gain from preparing early and executing later). The
     * driver call itself now happens where Stage 4's bind/execute used
     * to be, after the before-image/audit work below - sql_buf is
     * already built and hashed by this point, ready to hand to
     * dml_execute() once that stage is reached.
     *
     * Core only ever calls the vendor-neutral db_driver_get() - never
     * reaches for driver_oracle.h directly, same reasoning as the
     * SELECT integration. */
    const db_driver_t *driver = db_driver_get(ctx);
    if (!driver || !driver->dml_execute || !driver->commit || !driver->rollback)
    {
        logger_write(ctx->delete_logger, LOG_ERROR, __func__, 0,
     ...
     *  indicators allocation here - dml_execute() owns OCIBindByPos
     *  internally (driver_oracle.c). This stage now only needs to
     *  build the plain array of pointers dml_execute() expects -
     *  key_value strings are already null-terminated, caller-owned,
     *  and outlive the call (dc stays alive through Stage 4 below), so
     *  no copying/allocation is needed at all, unlike the old
     *  scalar_bufs array.
     * ================================================================ */
    logger_write(ctx->delete_logger, LOG_INFO, __func__, 0,
                 "Stage 3: Building bind value array keys=%d",
                 dc->key_count);

    const char *bind_values[MAX_DEL_KEY_COLS];
    for (int k = 0; k < dc->key_count; k++)
    {
        bind_values[k] = dc->keys[k].key_value;

        logger_write(ctx->delete_logger, LOG_DEBUG, __func__, 0,
                     "Bind key=%d name='%s' type='%s' value='%s' "
                     "bind_pos=%d",
    ...
    /* ================================================================
     *  Stage 4 - Execute the DELETE via the driver
     * ================================================================ */
    logger_write(ctx->delete_logger, LOG_INFO, __func__, 0,
                 "Stage 4: Executing DELETE via driver");

    clock_gettime(CLOCK_MONOTONIC, &ts_start);

    db_dml_request_t dml_req;
    ...
    logger_write(ctx->delete_logger, LOG_INFO, __func__, 0,
                 "driver dml_execute OK rows_deleted=%d", rows_deleted);
...
Cleanup:
    ...
    if (rc != 0 && rows_deleted > 0)
    {
        ...
        driver->rollback(ctx, ctx->delete_logger);
    }
