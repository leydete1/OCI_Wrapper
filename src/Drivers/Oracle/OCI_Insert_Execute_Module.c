                          ...
                          execute_config_t *cfg)
{
    ...
    /* Bug fix (2026-08-25) - RETURNING ROWID INTO array bind. One
     * ROWID string per row in the current batch, populated directly by
     * OCIStmtExecute() itself - see build_insert_sql()'s own doc
     * comment on why this replaces the old post-execute OCIAttrGet()
     * approach. ROWID_BUF_SIZE=24 - Oracle's extended ROWID format is
     * always exactly 18 characters; 24 leaves a safety margin. Buffer
     * is calloc'd (zeroed) so each row's slot is naturally NUL-
     * terminated after Oracle writes its fixed-length 18 characters,
     * with no return-length array needed - same simplicity the
     * existing scalar column binds already rely on. */
    #define ROWID_BUF_SIZE 24
    OCIBind       *rowid_bind_hdl   = NULL;
    char          *rowid_bufs       = NULL;   /* flat [row * ROWID_BUF_SIZE] */
    ...
    /* Bug fix (2026-08-25) - allocate the RETURNING ROWID INTO array
     * bind buffers once, sized to execute_count (the max rows any
     * single OCIStmtExecute call in this function will ever process) -
     * same allocation pattern as scalar_bufs[] just above. calloc, not
     * malloc, so every slot starts zeroed - see the declaration site's
     * own comment on why that matters for NUL-termination. */
    rowid_bufs = calloc((size_t)execute_count, (size_t)ROWID_BUF_SIZE);
    ...
    while (row_base < ic->row_count)
    {
        ...
        /* Bug fix (2026-08-25) - bind the RETURNING ROWID INTO output,
         * at the exact position build_insert_sql() placed its
         * placeholder (rowid_bind_num + 1 - see that function's own
         * comment). Same OCIBindByPos + OCIBindArrayOfStruct pattern
         * every scalar column above already uses - this is genuinely
         * just another array bind, OCI's RETURNING clause needs no
         * special bind mode beyond binding to the right position. */
        CHECK_OCI_INS(ctx->errhp,
            OCIBindByPos(stmt, &rowid_bind_hdl, ctx->errhp,
                         (ub4)(rowid_bind_num + 1),
                         rowid_bufs,
                         (sb4)ROWID_BUF_SIZE,
                         SQLT_STR,
                         rowid_indicators,
        ...
        /* Bug fix (2026-08-25) - the old "always obtain ROWID after
         * execute" block (OCIDescriptorAlloc/OCIAttrGet(OCI_ATTR_ROWID)/
         * OCIRowidToChar) is REMOVED here - it's now entirely obsolete.
         * rowid_bufs already holds one real, correct ROWID string per
         * row in this batch, populated directly by the OCIStmtExecute()
         * call above via the RETURNING ROWID INTO array bind - no
         * separate fetch needed, and (per build_insert_sql()'s own
         * comment) OCI_ATTR_ROWID was never reliable for a multi-row
         * statement in the first place.
         *
         * rowid_str (function-scoped, used by the audit block after the
         * main loop) is kept working exactly as before - it held
         * whatever the LAST batch's single fetched rowid was, so it's
         * set here to the last row's own rowid in the current batch,
         * preserving that same "most recently written row" semantic
         * without changing the audit trail's own behaviour, which is a
         * separate concern from this fix. */
        memset(rowid_str, 0, sizeof(rowid_str));
        if (batch_rows > 0)
        {
            const char *last_row_rowid =
                rowid_bufs + (size_t)(batch_rows - 1) * ROWID_BUF_SIZE;
         ...
         * Bug fix (2026-08-25) - this block used to only ever touch
         * ic->values[row_base * ic->col_count + bc] - i.e. ONLY the
         * first row of the batch, for every column, regardless of
         * batch_rows. Any row after the first in a multi-row LOB batch
         * got its BLOB/CLOB column silently left as an unfilled
         * EMPTY_BLOB()/EMPTY_CLOB() - real, confirmed silent data loss
         * for every batch insert with more than one row and at least
         * one LOB column. Fixed by wrapping the whole block in a real
         * per-row loop, using row_base+r for the value lookup and
         * rowid_bufs's own row-r slot (this row's OWN correct ROWID,
         * from the array bind above) instead of the single, batch-wide
         * rowid_str. has_blob is now a pure SCHEMA check (does this
         * table have any LOB column at all) rather than a data check on
         * row_base alone - the per-row is_empty check inside the loop
         * already handles skipping a genuinely empty LOB value on any
            ...
            if (has_blob_column)
            {
                ...
                for (int r = 0; r < batch_rows; r++)
                {
                    const char *row_rowid =
                        rowid_bufs + (size_t)r * ROWID_BUF_SIZE;
                    ...
                    for (int bc = 0; bc < ic->col_count; bc++)
                    {
                        ...
                        const insert_col_value_t *fv =
                            &ic->values[(row_base + r) * ic->col_count + bc];
...
/* ================================================================
    ...
    /* Bug fix (2026-08-25) - free the RETURNING ROWID INTO array bind
     * buffers allocated alongside scalar_bufs/indicators above.
     * rowid_bind_hdl itself needs no explicit free - OCI bind handles
     * are owned by the statement and released with it via
     * OCIStmtRelease(stmt, ...) further down, same as bind_hdls[]'s own
     * handles already were before this fix. */
