...
#include "db_driver.h"        /* v2 driver integration, 2026-09-19 -
                                  ...
                                  directly, same as SELECT/DELETE's own
                                  integrations.                        */
...
/* ================================================================== */
/*  handle_blob_update() / handle_clob_update() / dynamic_rowid_       */
/*  collector_t (init/free/rowid_in_callback/rowid_out_callback) were  */
/*  removed here (Phase 2, 2026-09-19) - no remaining caller anywhere  */
/*  in this file once the driver integration landed.                  */
/*                                                                      */
/*  execute_update_batch() now fetches through                        */
/*  driver->dml_execute_returning_rowids()/lob_write_by_rowid() (see   */
/*  the v2 driver integration comment inside execute_update_batch()    */
/*  itself), which do their own self-contained RETURNING ROWID         */
/*  collection and LOB writing (driver_oracle.c's own                  */
/*  oracle_dml_execute_returning_rowids()/oracle_lob_write_by_rowid(),  */
/*  themselves faithful ports of exactly these functions - see that    */
/*  file's own comments) and never touch a core-owned collector or     */
/*  bind-structure array.                                              */
/* ================================================================== */
                          ...
                          execute_config_t  *cfg)
{
    ...
    const char   **bind_values  = NULL;   /* built fresh in Stage 3, not
                                              inherited from the old
                                              bind_hdls/scalar_bufs/
                                              indicators trio - see that
                                              stage's own comment       */
    ...
    /* v2 driver integration (2026-09-19). The old, separate
     ...
     * RETURNING ROWID INTO is unconditional in every UPDATE's SQL
     * (confirmed directly against build_update_sql() - not just when a
     * LOB column is present), so EVERY update, scalar-only or not, goes
     * through this one driver call - there is no separate "simple"
     * dml_execute() path for UPDATE the way there might first appear to
     * be from this module's own Path-A/Path-B-shaped history; only
     * whether the post-execute LOB-write loop below actually runs
     * varies.
     *
     * Core only ever calls the vendor-neutral db_driver_get() - never
     * reaches for driver_oracle.h directly, same reasoning as SELECT/
     * DELETE's own integrations. */
     ...
     *  indicators allocation here - dml_execute_returning_rowids()
     *  owns OCIBindByPos/OCIBindDynamic internally (driver_oracle.c).
     *  This stage now only needs to build the plain array of pointers
     *  it expects - SET values first (skipping LOB columns, which
     *  still use EMPTY_BLOB()/EMPTY_CLOB() literals with no bind
     *  placeholder at all - core, i.e. build_update_sql(), decides
     *  that, unchanged by this pass), then WHERE key values, in
     *  exactly the same position order build_update_sql() already
     *  wrote into the SQL text itself.
     *
     *  fv->is_empty (a genuinely NULL SET value) maps to a NULL entry
     *  in bind_values - see db_dml_returning_request_t's own doc
     *  comment in db_driver.h for what the driver does with that
     *  (binds real SQL NULL, not empty string - a real fix made
     *  2026-09-19 specifically because this exact case needed it).
     * ================================================================ */
    logger_write(ctx->update_logger, LOG_INFO, __func__, 0,
                 "Stage 3: Building bind value array");

    int total_bind_cols = uc->col_count + uc->key_count;
    bind_values = calloc((size_t)total_bind_cols, sizeof(char *));
    if (!bind_values)
    {
        logger_write(ctx->update_logger, LOG_ERROR, __func__, 0,
    ...
    int scalar_bind_pos = 0;   /* 0-based array index - matches SQL's
                                   own 1-based :N positions offset by
                                   one, not a separate counter needing
                                   its own reasoning */
    ...
    for (int c = 0; c < uc->col_count; c++)
    {
        ...
        bind_values[scalar_bind_pos] = fv->is_empty ? NULL : upd_field_value_get(fv);

        logger_write(ctx->update_logger, LOG_DEBUG, __func__, 0,
                     "Bind SET col=%d name='%s' bind_pos=%d (%s)",
                     c, uc->col_names[c], scalar_bind_pos + 1,
    ...
    for (int k = 0; k < uc->key_count; k++)
    {
        bind_values[scalar_bind_pos + k] = uc->keys[k].key_value;

        logger_write(ctx->update_logger, LOG_DEBUG, __func__, 0,
                     "Bind WHERE key=%d name='%s' value='%s' bind_pos=%d",
                     k, uc->keys[k].field_name, uc->keys[k].key_value,
                     scalar_bind_pos + k + 1);
    }

    int total_scalar_binds = scalar_bind_pos + uc->key_count;
     ...
     *  uc->row_count is unconditionally 1 (confirmed directly against
     *  build_update_ctx_from_request(), the only place it's ever
     *  assigned - one line, no conditional), so that loop only ever
     *  ran once; this is now a straight-line sequence matching what
     *  actually happens, not what the old per-row-VALUE-array bind
     *  shape (OCIBindArrayOfStruct, also gone - batch_rows>1 never
     *  happened either) made it look like it might.
     * ================================================================ */
    logger_write(ctx->update_logger, LOG_INFO, __func__, 0,
    ...
    dml_req.sql                     = sql_buf;
    dml_req.bind_count              = total_scalar_binds;
    dml_req.bind_values             = bind_values;
    dml_req.returning_bind_position = rowid_bind_num;

    db_rowid_result_t rowid_result;
    if (driver->dml_execute_returning_rowids(ctx, ctx->update_logger,
    ...
    /* Bug fix (2026-09-19, closing the gap found during this pass's own
     * analysis): rows_updated now reads rowid_result.count - the REAL
     * number of physical rows Oracle's own RETURNING clause reported -
     * not a bind-iteration count that was always 1 regardless of how
     * many rows genuinely matched (confirmed directly: WHERE
     * NUMBER_COL=300 matching 209 rows used to report rows_affected=1
     * in the response). The LOB-writing fix already used the right
     * count (rowid_collector.count, now rowid_result.count) for its own
     * purposes since 2026-08-27 - this brings the RESPONSE's own
     * rows_affected to the same correctness, not a new behavior being
     * invented. */
    rows_updated = rowid_result.count;
    metrics.rows_affected = (uint64_t)rows_updated;

    logger_write(ctx->update_logger, LOG_INFO, __func__, 0,
                 "driver dml_execute_returning_rowids OK rows_updated=%d",
                 rows_updated);

    /* ---- Post-execute: write LOB data ----
     * Same reasoning as the 2026-08-25/27 fix this replaces (preserved
     * in spirit below): the real bound for how many rows need LOB
     * content is rowid_result.count, not a fixed batch shape - and the
     * same one set of SET-clause values goes to every row Oracle
     * reports as affected, matching what a normal UPDATE...WHERE
     * genuinely does. */
    {
        ...
        if (has_lob_column)
        {
            ...
            if (strlen(uc->owner) > 0)
                ...
                snprintf(tbl_fq, sizeof(tbl_fq), "%s", uc->table_name);

            logger_write(ctx->update_logger, LOG_INFO, __func__, 0,
                         "UPDATE affected %d row(s) - writing LOB "
                         "content to every one", rows_updated);

            /* Resolve each LOB column's value ONCE here, before the
             * per-row loop - not once per row, unlike the code this
             * replaces (handle_clob_update() used to re-open and
             * re-read the same file for every matched row; same
             * content, redundant work). A genuine small efficiency
             * improvement found while integrating, not just a refactor -
             * worth knowing about, not just silently changed. BLOB's
             * value is always a file path already (file_path field);
             * CLOB needs the same file://-prefix check
             * handle_clob_update() used to do internally, now done
             * once here instead. */
            for (int bc = 0; bc < uc->col_count; bc++)
            {
                ...
                if (is_clob)
                {
                    const char *raw = upd_field_value_get(fv);
                                ...
                                CLOB_FILE_PREFIX_LEN) == 0)
                    {
                        ...
                        if (!fp)
                        {
                            logger_write(ctx->update_logger, LOG_ERROR,
                                         __func__, 0,
                                         "Failed to open CLOB file: %s",
                                         file_path);
                            rc = -1;
                            driver->rowid_result_free(&rowid_result);
                            goto Cleanup;
                        }
                        fseek(fp, 0, SEEK_END);
                        long fsize = ftell(fp);
                ...
                for (int r = 0; r < rowid_result.count; r++)
                {
                    ...
                    if (is_blob)
                    {
                        ...
                        lob_rc = driver->lob_write_by_rowid(ctx, ctx->update_logger,
                                                             &lob_req, &bytes_written);
                        lob_bytes += bytes_written;
                    }
                    else
                    {
                        lob_req.inline_text     = resolved_text;
                        lob_req.inline_text_len = resolved_len;
                        lob_rc = driver->lob_write_by_rowid(ctx, ctx->update_logger,
                                                             &lob_req, &bytes_written);
                    ...
                    if (lob_rc != 0)
                    {
                        if (text_is_heap) free((void *)resolved_text);
                        rc = -1;
                        driver->rowid_result_free(&rowid_result);
                        goto Cleanup;
                    }
...
Cleanup:
    ...
    if (rc != 0 && rows_updated > 0)
    {
        ...
        driver->rollback(ctx, ctx->update_logger);
    }
