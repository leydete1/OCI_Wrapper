...
/* OCI_Connect_standalone()/OCI_Disconnect_standalone() - renamed
 * 2026-09-22 (from OCI_Connect()/OCI_Disconnect()) to make it visually
 * obvious at every call site that this is a single, non-pooled
 * connection, distinct from OCI_Connect_pool()/OCI_Disconnect_pool()
 * below - production traffic uses the pool exclusively
 * (config.ini's use_connection_pool=1); this path is only still
 * reachable via driver_oracle.c's oracle_connect()/oracle_disconnect()
 * when use_connection_pool=0 (an unused-in-practice but still-real
 * driver configuration, not deprecated here) and by two test harnesses
 * that need a genuinely independent connection on purpose
 * (Driver_Connect_Test.c and OCI_Unit_Test_Module.c's test_ut_conn_005,
 * which deliberately kills this connection to test the pool's dead-
 * session detection without touching the pool itself). Do not call
 * this from new production dispatch code - use the driver's connect()/
 * disconnect() (routes through the pool automatically) instead.       */
